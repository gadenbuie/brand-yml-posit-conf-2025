...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    div(
      ...
      p(tags$img(
        class = "img-fluid",
        src = file.path("brand", brand$logo$medium$path),
        alt = brand$logo$medium$alt
      )),
      p(
        "This project contains synthetic data and analysis created for demonstration purposes only."
      )
...
server <- function(input, output, session) {
  ...
  output$profit_curve <- renderPlotly({
    ...
    p <- ggplot(plot_data, aes(x = price, y = value, color = metric)) +
      ...
      geom_point(
        ...
        color = brand$color$palette$electric_blue,
        size = 6,
        inherit.aes = FALSE
      ) +
      geom_point(
        data = current_data,
        aes(x = price, y = costs),
        color = brand$color$palette$coral_red,
        size = 6,
        inherit.aes = FALSE
      ) +
      scale_color_manual(
        values = c(
          "revenue" = brand$color$palette$electric_blue,
          "costs" = brand$color$palette$coral_red
        ),
  ...
  output$segment_adoption <- renderPlotly({
    ...
    p <- ggplot(
      ...
      geom_col(fill = brand$color$palette$pulse_purple) +
      coord_flip() +
